...
$errors = '';
require_once TEMPLATEPATH . '/admin_processing/admin_specification_processing.php';

if (isset($_GET['action']) && ( 'edit' == $_GET['action'] || 'sorting' == $_GET['action']))
{
	require_once TEMPLATEPATH . '/admin/admin_edit_spec.php';	
}else
{
	...
	elseif(isset($_GET['cat']) && !empty($_GET['cat']))
		$cate_id = $_GET['cat'];
	$result = get_all_data_spec_by_cat_id_tgt($cate_id);
?>
<div class="wrap">
	...
	<div id="poststuff">
		<div class="postbox" id="specification_group" >
			...
			<div class="inside">				
				...
				<p>
					<form action="admin.php?page=specification" method="post" name="filter_by" id="filter_by">						
						<?php categories_dropdown( array( 'selected' => $cate_id )); ?>
						<a href="admin.php?page=specification&action=sorting&cat=<?php echo $cate_id; ?>"><?php _e('Sort groups for this category','re') ?></a>
					</form>
				</p>				
				<?php
				if(!empty($result))
						...
						<div class="list-item">
							<div class="list-header">
								...
								<span class="list-controls">
									<a href="#" class="list-toggle" title="<?php _e('Display specification item in this group','re') ?>" ><span><?php echo $v_g['group_name']; ?></span></a>
								</span>
							</div>
							<div class="list-content">
								...
								<ul class="group-list">
									...
									<li class="group-item">
										<form action="" method="post">
											...
											<input type="hidden" value="<?php echo $v_s['spec_value_id']; ?>" name="value_id" />
											...
											<span class="item-controls" style="position: inherit" >
												...
												<a class="item-control-delete" id="del_spec_<?php echo $v_s['spec_value_id']; ?>_group_<?php echo $v_g['spec_id']; ?>"><?php _e('Delete','re') ?></a>
											</span>										
								...
								<div class="new-group-value">									
									<div class="form-group-value">
										<form action="" method="post">											
											<input type="hidden" name="group_id" value="<?php echo $v_g['spec_id']; ?>" />
									...
									<p class="group-value-control">
										...
										<a href="admin.php?page=specification&action=edit&group=<?php echo $v_g['spec_id']; ?>" class="item-control-edit"><?php _e('Edit','re') ?></a> &nbsp;|&nbsp;
										<a class="item-control-delete" href="#" id="del_group_<?php echo $v_g['spec_id']; ?>"><?php _e('Delete','re'); ?></a>
										<?php
										if(!empty($v_g['value']))
										{
										?>
											&nbsp;|&nbsp;										
											<a href="admin.php?page=specification&action=sorting&cat=<?php echo $cate_id; ?>&group=<?php echo $v_g['spec_id']; ?>" class="item-control-sorting" ><?php _e('Sort values of group','re'); ?></a>
										<?php
										}
										?>
									</p>
...
jQuery(document).ready(function($){
	...
	jQuery('.item-control-delete').click(function(){		
		if(confirm('<?php _e('Are you sure want to delete this item ?','re'); ?>') == true)
		{
			...
			ajax_url = '<?php echo HOME_URL . '/?ajax=admin_spec_delete'?>';			
			$.post(
				reviewAjax.ajaxurl,
				{
					action: 'admin_spec_delete',
					para: this.id
				},
				function(data){
					if (data.success){
						if(data.para == 1)
						{
							alert('<?php _e('You deleted specification group successfully!','re'); ?>');
						}
						else if(data.para == 2)
						{
							alert('<?php _e('You deleted specification value successfully!','re'); ?>');
						}
						//alert(data.para);
						window.location = 'admin.php?page=specification';
					}
					else
						alert (data.para);
				}

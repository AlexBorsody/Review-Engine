...
$lastposts = tgt_lp_get_products();
?>
<div class="wrap"> <!-- #wrap 1 -->
	<div id="icon-edit" class="icon32"><br></div>
	<h2>
		<?php _e( 'List Products', 're' );?>
		<a class="button add-new-h2" href="<?php echo 'admin.php?page=review-engine-add_product';?>"><?php _e( 'Add New Product', 're' );?></a>
	</h2>
	...
	<form method="post" action="?page=review-engine-list-products&amp;to=1&amp;post_status=<?php tgt_lp_get_current_page();?>">	 <!-- form -->
			...
			<tbody> <!-- table body -->
				...
				if ( empty( $lastposts ) ) {
					...
					<tr valign="top" class="alternate author-other status-publish iedit" id="post-43">
	                    ...
	                    <strong><a title="<?php _e( 'Edit this item', 're' );?>" href="admin.php?page=review-engine-add_product&amp;p=<?php echo $post->ID;?>" class="row-title"><?php echo htmlspecialchars( $post->post_title );?> </a><b><?php echo ( $post->post_status == 'publish' ? '' : ' - ' . $post->post_status );?></b></strong>
						<div class="row-actions">
	                    <span class="edit"><a title="<?php _e( 'Edit this item', 're' );?>" href="admin.php?page=review-engine-add_product&amp;p=<?php echo $post->ID;?>"><?php _e( 'Edit', 're' );?></a> | </span>
	                    <span <?php echo tgt_lp_get_current_page(false) != 'trash'? 'class="trash"' : '';?>><a href="admin.php?page=review-engine-list-products&amp;productid=<?php echo $post->ID;?>&amp;post_status=<?php tgt_lp_get_current_page();?>&amp;action=<?php tgt_lp_get_action($post->post_status);?>" class="submitdelete"><?php _e( tgt_lp_get_action($post->post_status, true, false), 're' );?></a> |</span>
	                    <?php if ( tgt_lp_get_current_page(false) == 'trash') {?>
	                    <span class="trash"><a href="admin.php?page=review-engine-list-products&amp;productid=<?php echo $post->ID;?>&amp;post_status=<?php tgt_lp_get_current_page();?>&amp;action=delete" class="submitdelete"><?php _e( 'Delete', 're' );?></a> |</span>
	                    <?php }?>
	                    <span class="view"><a rel="permalink" title="<?php _e( 'View', 're' );?> '<?php echo $post->post_title;?>'" href="<?php echo get_post_permalink( $post->ID );?>"><?php _e( 'View', 're' ); ?></a></span></div>
							...
							<td class="author column-author"><a href="admin.php?page=review-engine-list-products&amp;post_status=<?php tgt_lp_get_current_page(false); ?>&amp;author=<?php echo $post->post_author;?>"><?php the_author();?></a></td>
							<td class="categories column-categories">
								...
								foreach($post_categories as $c){
									$cat = get_category( $c );
									$cats[] =  '<a href="admin.php?page=review-engine-list-products&amp;post_status=' . tgt_lp_get_current_page(false) . '&amp;cat=' . $cat->term_id . '">' . $cat->name . '</a>';
								}
								echo ( implode( ', ', $cats ) );
								?>
							</td>
							<td class="tags column-tags">
								...
								$post_tags = array();
								foreach( $tags as $tag ) {
									$post_tags[] = '<a href="admin.php?page=review-engine-list-products&post_status=' . tgt_lp_get_current_page(false) . '&tag=' . $tag->term_id . '">' . $tag->name . '</a>';
								}
								if ( ! empty ( $post_tags ) ) {
							...
							<td class="date column-date"><abbr title="<?php echo( $post->post_modified );?>"> <?php echo( $post->post_modified );?></abbr><br></td>	
					</tr>
					<?php } // end if?>
				<?php } // end for?>

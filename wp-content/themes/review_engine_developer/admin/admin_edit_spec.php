...
{	
	...
	{
		$result = get_all_data_spec_tgt($_GET['group']);
		...
		<div id="poststuff">
			<div class="postbox" id="specification_group" >
				<h3 class="hndle">
					<label for="">
						<?php echo $title; ?>
						<a href="admin.php?page=specification" class="button">
							<?php _e('Back','re') ?>
						</a>
					</label>
				</h3>
				<div class="inside">
					...
					if ($_GET['action'] == 'edit' && isset($_GET['group']) && !empty($_GET['group']) ) {
						$group_id = $_GET['group'];
						$spec_data = get_all_data_spec_tgt($group_id);						
					?>					
					<form action="" method="post" id="form_edit_group">
						...
						<p>
							...
							<a href="admin.php?page=specification" class="close_group"><?php _e('Cancel', 're'); ?></a>
						</p>
					</form>
					...
					}else
					{
						if($_GET['action'] != 'sorting')
							echo "<script language='javascript'>window.location = '"."admin.php?page=specification"."'</script>";
					}
					/**
					...
					{
						if ( empty( $_GET['group'] ) )
							printf( '<p>' . __('You are sorting specification groups in category <b>%s</b>. Drag and drop groups and click save','re') . '</p>', $cate_name );
						else
							printf( '<p>' . __('You are sorting specification values in group <b>%s</b>. Drag and drop groups and click save','re') . '</p>', $cate_name );
									...
									<li class="sortable-item">
										<div class="list-item">
											<div class="list-header">
												...
												<input type="hidden" name="sorting[]" value="<?php echo $v_g['spec_id']; ?>" />
											</div>
										</div>
									</li>
							<?php } ?>
